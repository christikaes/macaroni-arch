...
export default function DSMMatrix({ data }: DSMMatrixProps) {
  ...
  const HEADER_CELL_SIZE = Math.round(100 * zoom);
  const PATH_CELL_WIDTH = Math.round(500 * zoom); // Wide column for hierarchical paths
  const ID_CELL_WIDTH = Math.round(50 * zoom);
  ...
  const renderPathWithConnectors = useCallback((item: DisplayItem) => {
    ...
    return (
      <div className="flex items-center whitespace-nowrap" style={{ fontFamily: "var(--font-geist-mono)", fontSize: zoom < 0.6 ? "0" : zoom < 0.8 ? "10px" : "12px" }}>
  ...
  const Cell = useCallback(({ columnIndex, rowIndex, key, style }: CellProps) => {
    ...
    const cellBorderWidth = zoom < 0.3 ? 0.5 : zoom < 0.6 ? 1 : 1;
    ...
    if (showHeaders && adjustedRowIndex === 0) {
      ...
      if (adjustedColumnIndex === pathColIndex) {
        return (
          <div key={key} style={{ ...style, borderWidth: cellBorderWidth }} className="bg-yellow-100 border-gray-300 flex items-center justify-center font-semibold">
            <span style={{ fontSize: zoom < 0.6 ? "8px" : "10px" }}></span>
          </div>
        );
      }
      // ID column header
      ...
      return (
        <div key={key} style={{ ...style, borderWidth: cellBorderWidth }} className="bg-yellow-100 border-gray-300 flex items-center justify-center" title={item.path}>
          <div style={{ writingMode: "vertical-rl", transform: "rotate(180deg)", fontSize: zoom < 0.6 ? "8px" : "10px" }}>
            {zoom >= 0.6 && item.id}
          </div>
        </div>
      );
    ...
    if (showHeaders && adjustedColumnIndex === idColIndex) {
      return (
        <div key={key} style={{ ...style, borderWidth: cellBorderWidth, borderRightWidth: 2 }} className="bg-yellow-50 border-gray-300 border-r-black flex items-center justify-center text-xs px-1" title={rowItem.path}>
          <span style={{ fontSize: zoom < 0.6 ? "0" : zoom < 0.8 ? "8px" : "10px" }}>
            {zoom >= 0.6 && rowItem.id}
          </span>
        </div>
      );
    ...
    const borderWidth = zoom < 0.3 ? 1 : 2;
  ...
  }, [matrixItems, getDependencyCount, fileList, files, zoom, renderPathWithConnectors, getAncestorFolders, getDependencyColor, hoveredCell, tooltip]);
  ...
  return (
    <div className="flex flex-col" style={{ width: `${actualWidth}px` }}>
      ...
      <div ref={containerRef} className="flex-1 overflow-hidden">
        <MultiGrid
          key={`grid-${zoom >= 0.6 ? 'fixed' : 'nonfixed'}`}
          ref={gridRef}
          ...
          fixedColumnCount={zoom >= 0.6 ? 2 : 0}
          fixedRowCount={zoom >= 0.6 ? 1 : 0}
          height={actualHeight}
          rowCount={rowCount}
          rowHeight={({ index }: { index: number }) => (zoom >= 0.6 && index === 0) ? HEADER_CELL_SIZE : CELL_SIZE}
          width={actualWidth}
          overscanRowCount={10}
